...
#import "MCTagsControl.h"

@interface MCTagsControl () <UITextFieldDelegate, UIGestureRecognizerDelegate>

@end

@implementation MCTagsControl {
...
- (void)layoutSubviews {
    ...
    for (UIView *view in tagSubviews_) {
        ...
        if (_mode == MCTagsControlModeList) {
            view.tag = tagIndex;
    ...
    contentSize.height = self.frame.size.height - self.contentInset.top - self.contentInset.bottom;
...
- (void)setMode:(MCTagsControlMode)mode {
    _mode = mode;
}
